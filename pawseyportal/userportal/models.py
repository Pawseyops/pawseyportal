...
class Project(models.Model):
    ...
    altAdmins = models.ManyToManyField(Person, blank=True, verbose_name = 'Alternative Administrators', related_name='admin_people', help_text = help_text.project_altAdmins)
...
class Allocation(models.Model):
    ...
    def quarterLength(self):
        return ( ((self.end.year - self.start.year) * 4) +  ((((self.end.month-1)//3 + 1) - ((self.start.month-1)//3 + 1)) + 1) )
